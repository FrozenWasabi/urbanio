class Bus {
  ...
  void moveBus() {
    ...
    //if at intersection and not already turning, turn
    if (isTurning == false && turningCooldown == 0) {
      for ( int i = 0; i < 5; i++) {
        if (dist(this.getCenterLocationX(),this.getCenterLocationY(),allRoads.get(i+5).getCenterLocationX(),allRoads.get(i+5).getCenterLocationY()) <= 90) {
          isTurning = true;
          turningLeft = true;
        }
      }
  ...
  float getCenterLocationX() {
    float middleX = this.xPos + this.vWidth/2 - 170;
    return middleX;
  }
  
  float getCenterLocationY() {
    float middleY = this.yPos + this.vHeight/2 - 30;
    return middleY;
  }
